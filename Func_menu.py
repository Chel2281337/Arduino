...
ACTIVATE_BUTTON_SIZE = (400, 90)      # размер кнопки (w, h)
...
CONFIRM_CANCEL_BUTTON_SIZE = (250, 60)  # размер кнопки отмены
CONFIRM_CANCEL_OFFSET = (0, 20)         # смещение кнопки отмены относительно кнопки подтверждения
...
# параметры отображения топливных баков
FUEL1_IMG_PATH = os.path.join("img", "fuel1.png")
FUEL1_IMG_SIZE = (220, 180)           # базовый размер первой иконки
FUEL1_IMG_OFFSET = (-60, 32)          # смещение первого бака относительно панели
FUEL2_IMG_PATH = os.path.join("img", "fuel2.png")
FUEL2_IMG_SIZE = (220, 180)           # базовый размер второй иконки
FUEL2_IMG_OFFSET = (80, 32)           # смещение второго бака относительно панели
FUEL1_LEVEL_PERCENT = 73              # процент заполнения первого бака
FUEL2_LEVEL_PERCENT = 4             # процент заполнения второго бака
FUEL_PCT_OFFSET = (-20, 95)          # смещение процента первого бака
FUEL_PCT2_OFFSET = (-20, 95)         # смещение процента второго бака
FUEL_CAPTION_LINES = ("Уровень", "топлива")
FUEL_CAPTION_OFFSET = (-92, 13)       # смещение подписи относительно баков
FUEL2_CAPTION_LINES = ("Уровень", "заряда")
FUEL2_CAPTION_OFFSET = (-24, 13)      # смещение подписи второго бака

# смещения подписи состояний топливных баков и генераторов (dx, dy)
FUEL_STATUS_OFFSET = (-140, -85)
GEN_STATUS_OFFSET = (-140, -160)
...
FUEL1_IMG = pygame.image.load(FUEL1_IMG_PATH).convert_alpha()
FUEL2_IMG = pygame.image.load(FUEL2_IMG_PATH).convert_alpha()
...
map2_confirm = None  # {index, checked}

# обратный отсчёт после активации всех зарядов
countdown = None  # {'value': int, 'last': int, 'flash': int}

# финальное подтверждение перед запуском обратного отсчёта
final_confirm = None  # {'checked': [bool,bool,bool], 'boxes': [], 'texts': [], 'btn': Rect, 'cancel': Rect}
...
ASTEROID_FADE_SPEED = 120  # скорость исчезновения рамки и координат (alpha/сек)
...
# более мелкий шаг делает хвост радара визуально однородным
...
def spawn_asteroid(center, radius):
    ...
    rot_speed = random.uniform(-30, 30)  # degrees per second
    # ищем направление, которое не проходит через центр радара
    for _ in range(20):
...
def draw_radar(surface, rect, t):
    ...
    # однородный градиентный хвост за линией сканирования
...
def draw_energy_panel(surface, rect, t):
    ...
    energy_label = font_energy.render("Генераторы", True, blink_color)
    ...
    # иконки топливных баков с процентом топлива
    base_y = y
    fuel_img = pygame.transform.smoothscale(FUEL1_IMG, FUEL1_IMG_SIZE)
    center_x = x + SUBSYS_BAR_WIDTH // 2 + FUEL1_IMG_OFFSET[0]
    fuel_top = base_y + FUEL1_IMG_OFFSET[1]
    fuel_rect = fuel_img.get_rect(midtop=(center_x, fuel_top))
    surface.blit(fuel_img, fuel_rect)
    pct = font_small.render(f"{FUEL1_LEVEL_PERCENT}%", True, CYAN)
    pct_rect = pct.get_rect(center=(fuel_rect.centerx + FUEL_PCT_OFFSET[0],
                                    fuel_rect.centery + FUEL_PCT_OFFSET[1]))
    surface.blit(pct, pct_rect)

    fuel_img2 = pygame.transform.smoothscale(FUEL2_IMG, FUEL2_IMG_SIZE)
    center_x2 = x + SUBSYS_BAR_WIDTH // 2 + FUEL2_IMG_OFFSET[0]
    fuel_top2 = base_y + FUEL2_IMG_OFFSET[1]
    fuel_rect2 = fuel_img2.get_rect(midtop=(center_x2, fuel_top2))
    blink = (pygame.time.get_ticks() // 500) % 2 == 0
    if blink:
        tinted = fuel_img2.copy()
        overlay = pygame.Surface(tinted.get_size(), pygame.SRCALPHA)
        overlay.fill((*ALERT_RED, 255))
        tinted.blit(overlay, (0, 0), special_flags=pygame.BLEND_RGBA_MULT)
        surface.blit(tinted, fuel_rect2)
    else:
        surface.blit(fuel_img2, fuel_rect2)
    color2 = ALERT_RED if blink else CYAN
    pct2 = font_small.render(f"{FUEL2_LEVEL_PERCENT}%", True, color2)
    pct2_rect = pct2.get_rect(center=(fuel_rect2.centerx + FUEL_PCT2_OFFSET[0],
                                      fuel_rect2.centery + FUEL_PCT2_OFFSET[1]))
    surface.blit(pct2, pct2_rect)

    mid_x = (fuel_rect.centerx + fuel_rect2.centerx) // 2 + FUEL_CAPTION_OFFSET[0]
    cap_y = min(fuel_rect.top, fuel_rect2.top) + FUEL_CAPTION_OFFSET[1]
    y_line = cap_y
    for line in reversed(FUEL_CAPTION_LINES):
        txt_line = font_energy2.render(line, True, CYAN)
        surface.blit(txt_line, txt_line.get_rect(midbottom=(mid_x, y_line)))
        y_line -= txt_line.get_height()

    cap2_x = fuel_rect2.centerx + FUEL2_CAPTION_OFFSET[0]
    cap2_y = fuel_rect2.top + FUEL2_CAPTION_OFFSET[1]
    y_line2 = cap2_y
    for line in reversed(FUEL2_CAPTION_LINES):
        txt_line2 = font_energy2.render(line, True, CYAN)
        surface.blit(txt_line2, txt_line2.get_rect(midbottom=(cap2_x, y_line2)))
        y_line2 -= txt_line2.get_height()

    y = max(fuel_rect.bottom, fuel_rect2.bottom) + 20
...
def main():
    ...
    while True:
            ...
            if event.type == pygame.QUIT:
                ...
                if final_confirm:
                    for i, box in enumerate(final_confirm['boxes']):
                        text_rect = final_confirm['texts'][i]
                        if box.collidepoint(event.pos) or text_rect.collidepoint(event.pos):
                    ...
                    if (
                        ...
                        final_confirm = None
                    elif final_confirm['cancel'] and final_confirm['cancel'].collidepoint(event.pos):
                        final_confirm = None
                        for z in zaryads:
                            z['activated'] = False
                            z['anim_start'] = None
        ...
        if countdown is None and final_confirm is None and all(z['activated'] for z in zaryads):
            final_confirm = {'checked': [False, False, False], 'boxes': [], 'texts': [], 'btn': None, 'cancel': None}
        ...
        if final_confirm:
            ...
            texts = [
                ...
                "Я готов ко всему после нажатия",
            ]
            final_confirm['boxes'] = []
            final_confirm['texts'] = []
            y = panel.top + 40
            for i, line in enumerate(texts):
                ...
                txt = font_confirm_text.render(line, True, CYAN)
                txt_rect = txt.get_rect(midleft=(box.right + 10, box.centery))
                screen.blit(txt, txt_rect)
                final_confirm['boxes'].append(box)
                final_confirm['texts'].append(txt_rect)
                y += 40
            btn = pygame.Rect(0, 0, ACTIVATE_BUTTON_SIZE[0], ACTIVATE_BUTTON_SIZE[1])
            btn.center = (panel.centerx, panel.bottom - ACTIVATE_BUTTON_SIZE[1] // 2 - 20)
            ...
            cancel = pygame.Rect(0, 0, CONFIRM_CANCEL_BUTTON_SIZE[0], CONFIRM_CANCEL_BUTTON_SIZE[1])
            cancel.centerx = panel.centerx + CONFIRM_CANCEL_OFFSET[0]
            cancel.top = btn.bottom + CONFIRM_CANCEL_OFFSET[1] + 20
            pygame.draw.rect(screen, CYAN, cancel, 2, border_radius=8)
            cancel_txt = font_activate_btn.render("Отмена", True, CYAN)
            screen.blit(cancel_txt, cancel_txt.get_rect(center=cancel.center))
            final_confirm['cancel'] = cancel
