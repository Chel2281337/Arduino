...
# исходный размер карты 12345.png
MAP_ORIGINAL_SIZE = (1024, 768)
# ширина области под легенду слева
LEGEND_WIDTH = 300
# размер шрифта легенды (можно менять при необходимости)
LEGEND_FONT_SIZE = 33
...
SCROLLBAR_WIDTH = 12
MAP_SCALE = 1  # fraction of the screen area used for the main map
...
CUSTOM_LABEL_POS = (0, 0)  # (x, y)

ERROR_DURATION = 4500
ERROR_BLINK_MS = 600

# попытка подключения к последовательному порту; если порт недоступен,
# работа продолжается без него
# ??? настройка подключения Arduino
try:  # ???
    ser = serial.Serial('COM3', 9600)  # ???
    time.sleep(2)  # ???
except Exception:  # ???
    ser = None  # ???
# ??? buffer for assembling incoming serial lines without blocking
serial_buffer = ""  # ???
# ------------------------------------------------------------------
...
# настраиваемые подписи для иконок
ICON_NAMES = {
    "terminal": "Терминал",
    "ruki": "Роборука",
    "server": "Сервер",
    "krio": "Криокамера",
    "rul": "Управление кораблём",
}

# элементы иконок легенды: (название, путь)
legend_icon_items = [
    (f"-  {ICON_NAMES['terminal']}", "img/terminal.png"),
    (f"-  {ICON_NAMES['ruki']}", "img/ruki.png"),
    (f"-  {ICON_NAMES['server']}", "img/server.png"),
    (f"-  {ICON_NAMES['krio']}", "img/krio.png"),
    (f"-  {ICON_NAMES['rul']}", "img/rul.png"),
    (f"-  {ICON_NAMES['ruki']}", "img/ruki.png"),
    (f"-  {ICON_NAMES['server']}", "img/server.png"),
    (f"-  {ICON_NAMES['krio']}", "img/krio.png"),
]
...
RUL_ICON_SIZE = 50
COM_ICON_SIZE = 50
LOCK_ICON_SIZE = 29  # размер иконки замка по умолчанию

# вертикальные смещения замков относительно центра двери (в пикселях карты)
LOCK_OFFSET_VERTICAL = 0
LOCK_OFFSET_HORIZONTAL = -6
DOOR_HITBOX_PADDING = 0  # дополнительный отступ зоны нажатия вокруг дверей
...
def show_error(message, draw_scene, blink_draw):
    ...
    while pygame.time.get_ticks() - start < ERROR_DURATION:
        ...
        screen.blit(msg, msg.get_rect(center=(err_rect.centerx, err_rect.bottom - 40)))
        pygame.display.flip()
        clock.tick(60)
...
class Door:
    ...
    def update_rect(self, map_rect):
        ...
        self.hit_rect = self.rect.inflate(DOOR_HITBOX_PADDING * 2, DOOR_HITBOX_PADDING * 2)
        if self.open:
...
horizontal_doors = [
    Door((474, 249, 62, 13), 'h', password="1111",
         open_message="A",
         close_message="B",
         lock_size=LOCK_ICON_SIZE),
...
# соответствие буквенных команд (A,B,...) дверям
# ??? карта команд -> двери
...
def poll_serial_commands():  # ???
    """Read commands from serial port and trigger door actions."""  # ???
    if not ser:  # ???
        return  # ???
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if scroll_thumb_rect.collidepoint(event.pos):
                    ...
                    for d in doors:
                        if d.hit_rect.collidepoint(event.pos):
                            d.handle_click()
